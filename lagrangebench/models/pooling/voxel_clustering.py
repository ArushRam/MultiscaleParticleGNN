...
class VoxelClustering(BasePooling):
    ...
    def __call__(self, positions: jnp.ndarray, particle_type: jnp.ndarray) -> Tuple[Dict[str, jnp.ndarray], jnp.ndarray]:
        # NOTE [Original input] features: Dict[str, jnp.ndarray]
        ...
        for ptype in jnp.unique(particle_type):
            positions_ptype = positions[particle_type == ptype]
            
            # 1. map each node position to an index
            voxel_indices = ((positions_ptype - self.bounds_min) // self.voxel_size).astype(int)
